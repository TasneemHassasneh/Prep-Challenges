...
const LastWord = (str) => {
  ...
  // Use the slice method to extract the last word from the string
...
const replaceWords = (str) => {
  ...
  for (let i = 0; i < words.length; i++) {
    // Check if the word is "I" and replace it with "We"
    if (words[i] === 'I') {
      words.splice(i, 1, 'We');
    }
    // Check if the word is "am" and replace it with "are"
    else if (words[i] === 'am') {
      words.splice(i, 1, 'are');
    }
    // Check if the word is "was" and replace it with "were"
    else if (words[i] === 'was') {
